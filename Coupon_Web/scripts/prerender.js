...
async function generateRoutes() {
  ...
  try {
    ...
    if (!productsRes.ok) {
      console.error(
        `[Pre-render] Failed to fetch products: HTTP error! status: ${productsRes.status}`
      );
      return staticRoutes;
    }
    ...
    const products = Array.isArray(rawProductsData)
      ? rawProductsData
      : rawProductsData.results || [];

    const productRoutes = products.map(
      (p) => `/store/${p.id}/${slugify(p.name)}/`
    );
    ...
    if (error.name === "AbortError") {
      console.error(
        "[Pre-render] Error fetching dynamic routes: Fetch timed out (cold start likely)."
      );
    } else {
      console.error(
        `[Pre-render] Error fetching dynamic routes: ${error.message}`
      );
    }
