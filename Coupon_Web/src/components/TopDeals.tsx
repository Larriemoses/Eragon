  ...
  useEffect(() => {
    const fetchTopDealsData = async () => {
      ...
      try {
        ...
        const fetchedProducts: Product[] = Array.isArray(productsData)
          ? productsData
        ...
        const fetchedCouponsRaw: any[] = Array.isArray(couponsData)
          ? couponsData
          : couponsData.results || [];

        let enrichedCoupons: ProductCoupon[] = fetchedCouponsRaw.map(
          (coupon) => {
            const productDetail = fetchedProducts.find(
              (p) => p.id === coupon.product
            ) || {
              id: coupon.product,
        ...
        if (localStorage.getItem(forceResetKey) === null) {
          console.log(
            "TopDeals: Performing one-time force reset of 'used_today'."
          );
        ...
        if (lastVisitDate !== today) {
          console.log(
            "DEBUG(TopDeals): New day detected. Resetting 'used_today' for all coupons."
          );
          ...
          console.log(
            "DEBUG(TopDeals): Same day. 'used_today' values are preserved (unless force-reset)."
          );
        }
        ...
        console.error(
          "Failed to fetch top deals data in TopDeals component:",
          error
        );
  ...
  const topDealsCoupons: ProductCoupon[] = React.useMemo(() => {
    ...
    coupons.forEach((coupon) => {
      if (coupon.product && typeof coupon.product.id === "number") {
        ...
        if (
          !currentBest ||
          coupon.used_count + coupon.used_today >
            currentBest.used_count + currentBest.used_today
        ) {
          productToBestCouponMap.set(coupon.product.id, coupon);
        }
      } else {
        console.warn(
          "DEBUG(TopDeals): Skipping coupon due to invalid product ID:",
          coupon
        );
      }
    });
    ...
    selectedCoupons.sort(
      (a, b) => b.used_count + b.used_today - (a.used_count + a.used_today)
    );
      ...
      <div className="max-w-xl w-[90%] mx-auto flex flex-col gap-6">
        {topDealsCoupons.length === 0 && (
          <div className="text-gray-500 text-center py-8">
            No top deals available from unique products.
          </div>
        )}
        {topDealsCoupons.map((coupon) => {
          ...
          const buttonText =
            product.is_signup_store === true ? "Sign Up" : "Shop Now";
              ...
              <div className="flex justify-start mb-2">
                {logoSrc ? (
                  <img
                    ...
                    draggable={false}
                    onError={(e) => {
                      e.currentTarget.src = `https://placehold.co/64x64/cccccc/ffffff?text=${product.name.charAt(
                        0
                      )}`;
                ...
                <div className="flex flex-wrap gap-2 items-center mt-auto">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      fetch(
                        `${BACKEND_URL}/api/productcoupon/${coupon.id}/use/`,
                        { method: "POST" }
                      )
                        .then(() => {
                          /* Backend will update its counts, no need to refetch all here */
                        })
                        .catch((err) =>
